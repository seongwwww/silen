...
**다음 후보:** 일기 편집·확정 · 날짜 이동/목록 · 기록 열람 · 사진 첨부.
...
| **12** | **일기 편집·확정 · 날짜 이동/목록 · 기록 열람 · 사진 첨부** | 🔧 **다음 착수** | — |
| — | 스케줄러 실제 등록(운영) | ⬜ 사람이 실행 | `README.md` §4 |

## 지금 브랜치

- `main` (병합 지점 `e0e701d`, 일기 보기 화면 merge). **origin/main과 동기화됨.** 검증: 프론트 단위 **49** · 통합 **44** · 워커 **113** · lint·build·ruff clean.
- 다음: **#12**(일기 편집·확정 등)를 새 기능 브랜치(`feat/…`)로 분기해 착수. 표준 루프(`AGENTS.md`의 워크플로: 브레인스토밍 → 계획 → 브랜치 → TDD → 리뷰 → PR).
- 미해결 사소 항목:
