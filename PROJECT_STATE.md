...
그리고 더 근본적으로, **탐지가 사용자가 이미 아는 것을 되돌려주고 있었다.**
사흘 연속 김밥을 먹은 걸 모르는 사람은 없다. 놓치기 쉬운 것은
**기록에서 뜸해진 것**·**오랜만인 것**·**흩어져 있어 못 묶는 것**이다.

**지금부터는 "돌아가게" 만드는 것만 한다.** 다만 통합 테스트 격리와 삭제·검색
보안 리뷰는 완료를 증명하는 안전장치이므로 기능과 함께 처리한다.
...
**현재 상태:** v2 설계 승인 완료. `fix/test-isolation`에서 Phase -1 구현과
전체 검증을 마쳤고 미커밋 상태다. 다음은 리뷰·커밋·병합 후 Phase 0~1이다.

| Phase | 내용 | 와이어프레임 |
|---|---|---|
| **-1 완료** | 통합 테스트 격리 — 전역 사용자/메일 삭제·큐 purge 제거, 사용자별 claim·cleanup | — |
| **0** | 언어(`lang="ko"`)·베이지 토큰 · **하단 탭바** | — |
| **1** | 기록을 `/record`로 분리 · **홈을 발견 화면으로** | #1 #2 |
| **2** | **탐지 재설계** — 놀라움(bits) · **기록 부재** · **감정축** · 랭킹·상한·기각학습 | #4 |
| **3** | 일기 **opt-out 전환** · 기본 프리셋·1회 톤 주문 | #3 |
| **4** | **주간 리포트** | #5 |
| **5** | **회고 — 키워드 검색** | #6 |
| **6** | 내보내기 · 전체 삭제 · `stats` · 보안 리뷰 | #7 |

**최소 마이그레이션 2건.** 감정 차이(`entity_id is null`)의 멱등 부분 인덱스와,
