...
마이그레이션 2건이 로컬에 미적용이고, 파괴적 삭제 통합 테스트와 실제 Vertex
eval은 사람 승인 전이라 실행하지 않았다. 이 세 게이트 전에는 MVP 완료로
표시하지 않는다.
...
마이그레이션·파괴 테스트·실제 LLM eval 승인 게이트가 남았다.
...
| **4 코드 완료** | **주간 리포트** | #5 |
| **5 코드 완료** | **회고 — 키워드 검색** | #6 |
| **6 코드 완료·검증 대기** | 내보내기 · 전체 삭제 · `stats` · 보안 리뷰 | #7 |
