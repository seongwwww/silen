...
## 실 Vertex 점검에서 나온 것 (2026-07-29)

**고침:**
- 탐지가 실패했는데 일기가 만들어졌다. 차이 0건짜리 "평범한 하루"로 굳고
  자동 재생성이 없어 되돌릴 수 없었다 → **마감 탐지가 성공해야 일기를 예약한다**
- 잠근 기록의 원문이 **이미 만들어진 일기 본문에 남았다**(프라이버시 반쪽)
  → 잠글 때 그 일기에 재생성 요청을 남긴다
- 화면의 "다시 만들기" 요청을 `run`이 처리하지 않아 영원히 대기했다
  → sweep이 주기마다 소비한다
- 워커가 자정을 넘겨 재시작하면 **전날이 영영 마감되지 않았다** → 어제를 따라잡는다
- 사진만 남긴 기록이 홈에서 **빈 줄**로 보였다 → "사진 한 장"

**effective_at (3fdfea2):** `occurred_at`이 탐지에 전혀 반영되지 않던 것을
생성 열로 고쳤다. `captured_at`은 감사·정렬, `effective_at`은 도메인 전부.
미래 시각은 경계에서 400.

**다음 작업: 과거 기록 유입 시 재계산 배선** —
`docs/superpowers/plans/2026-07-30-effective-at-recalc.md`.
날짜는 제자리를 찾았지만 그날의 차이·일기·리포트는 아직 옛 계산 그대로다.

**남은 것(미해결):**
- 회고 응답이 **21~44초**로 느리다
- 사진 업로드 뒤 메모 저장이 실패하면 **고아 파일**이 남는다
- 일반적인 업무 단어가 강한 부재 카드로 올라와 감정 카드를 밀어낸다(튜닝)

## 환경 (Windows) — 이미 밟은 지뢰, 반복하지 마라

0. **★ 환경변수와 워커를 같은 터미널에서 실행해라.** PowerShell 환경변수는
   다른 터미널로 전달되지 않는다. env 없이 워커를 띄우면 추출·임베딩이
   **조용히 실패하고 5회 뒤 잡이 보관된다. 보관된 잡은 자동 복구되지 않는다** —
   화면은 계속 "평소를 익히는 중이에요"에 머문다. 이 증상이면 env부터 봐라.
...
worker\.venv\Scripts\python.exe -m silen_worker run   # 이것 하나면 전부 자동
```
