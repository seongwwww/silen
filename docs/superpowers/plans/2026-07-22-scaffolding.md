...
**`python`이 아니라 `py -3.12`를 쓴다.** 이 환경에서 `python`은 anaconda 3.8.8을 가리켜 `zoneinfo`가 없다. venv를 3.12로 만들고 나면 이후 단계는 모두 `worker\.venv\Scripts\python.exe`를 직접 호출하므로 버전 혼동이 없다.
...
# Python 워커 (3.11+ 필요. `python`이 다른 버전을 가리킬 수 있으므로 py launcher 사용)
py -3.12 -m venv worker\.venv
worker\.venv\Scripts\python.exe -m pip install -e "worker[dev]"
