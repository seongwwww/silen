...
4. **`HANDOFF.md`** — **지금 당장 할 활성 작업·실행 방식·현재 상태**(여러 AI가 왔다갔다 하는 단일 인수인계 허브).
5. **`PROJECT_STATE.md`** — 전체 로드맵·현재 단계.
6. 손대는 기능의 스펙·계획: `docs/superpowers/specs/`, `docs/superpowers/plans/`.
7. 최근 히스토리: `git log --oneline -15`.

그다음 **`HANDOFF.md`의 "현재 활성 작업"**을 이어간다.
...
- **세션이 한도에 근접(≈80%)하거나 작업을 멈출 때**, 다음 에이전트가 즉시 이어받도록 정리한다:
  1. **`HANDOFF.md`의 "상태" 절을 갱신**(완료 태스크·커밋 SHA·다음 시작점·막힘). 이게 공용 진행 상태의 기준이다.
  2. 기능 단계가 바뀌었으면 `PROJECT_STATE.md`(로드맵)도 갱신.
  3. Claude가 SDD로 돌 땐 `.superpowers/sdd/progress.md` 원장에 더 세밀히 기록(선택).
  4. 새 설계 결정은 스펙/계획 문서 또는 `docs/decisions/ADR-xxxx.md`에 남긴다.
  5. 문서·설정 변경은 커밋한다(문서·설정은 main 직접 커밋 허용, `.claude/rules/git.md`). 코드는 브랜치/PR로.
