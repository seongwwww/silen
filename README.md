...
큐 소비(추출·임베딩·일기·회고)와 주기 점검(차이 탐지·서술·일기 예약·주간 리포트)을
함께 돈다. 사용자가 화면에서 아무것도 누르지 않아도 여기서 다 일어난다.

**탐지는 두 단계다.** 하루 중간에는 오늘 등장한 것에서 나오는 차이(재등장 등)만
만들고, 사용자 로컬 `diary_hour`가 지나면 부재·감정까지 포함해 마감한다 —
아직 끝나지 않은 하루에 "오늘 운동이 없네요"라고 말하면 사실이 아니다.

멱등은 자연키가 보장한다(`differences`는 (user,date,entity,method), 일기 요청은
(user,date), 주간은 (user,week)). 반복 실행이 중복을 만들지 않는다.

아래 개별 명령은 **디버깅·재실행용**으로 남겨 둔다.
